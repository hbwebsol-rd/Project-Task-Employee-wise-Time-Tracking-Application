...
// display all tasks
router.get('/getTasks', superUser, async(req, res)=>{
    try {
        // filters
...
router.get('/getTask/:task_id', superUser, async(req, res)=>{
    try {
        // check url task id
...
// add a task
router.post('/addTask', [superUser, [
    check('taskName'), check('projectId'), check('employeeId'), check('priority'), check('status')
]], async(req, res)=>{
...
// update a task
router.put('/updateTask/:task_id', [superUser, [
    check('taskName'), check('projectId'), check('employeeId'), check('priority'), check('status')
]], async(req, res)=>{
    const {taskName, employeeId, projectId, priority, status}=req.body
    const errors=[]
...
router.delete('/deleteTask/:task_id', superUser, async(req, res)=>{
    try {
        // check url task id
        if(!mongoose.Types.ObjectId.isValid(req.params.task_id)) return res.status(400).json({message: 'Invalid Task Id', success: false})
