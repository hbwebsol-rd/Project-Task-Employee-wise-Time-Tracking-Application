...
router.get('/getCustomers', superUser, async(req, res)=>{
    try {
        // filters
...
router.get('/getCustomer/:customer_id', superUser, async(req, res)=>{
    try {
...
// add a customers detail
router.post('/addCustomer', [superUser, [
...
// update a customers detail
router.put('/updateCustomer/:customer_id', [superUser, [
    check('name'), check('email')
]], async (req, res)=>{
    const {name, email}=req.body
    const errors=[]
...
// delete a customers details
router.delete('/deleteCustomer/:customer_id', superUser, async(req, res)=>{
    try {
        // check url customer id
        if(!mongoose.Types.ObjectId.isValid(req.params.customer_id)) return res.status(400).json({message: 'Invalid Customer Id', success: false})
