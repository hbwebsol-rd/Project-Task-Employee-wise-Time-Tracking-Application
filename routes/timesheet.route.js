...
module.exports=(app)=>{
    ...
    app.get('/api/getCustomTimesheets', [Auth.multiAccess, [check('startDate'), check('endDate')]], Timesheet.getCustomTimesheets)

    // create timesheet
    app.post('/api/addTimesheet', [Auth.employee, [check('taskId'), check('date'), check('startTime'), check('endTime')]], Timesheet.createTimesheet)

    // update timesheet
    app.patch('/api/updateTimesheet/:timesheet_id', [Auth.employee, [check('employeeId'), check('taskId'), check('date'), check('startTime'), check('endTime')]], Timesheet.updateTimesheet)
