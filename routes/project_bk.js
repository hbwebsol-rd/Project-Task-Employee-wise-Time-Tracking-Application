...
router.get('/getProjects', superUser, async(req, res)=>{
    try {
        // filters
...
router.get('/getProject/:project_id', superUser, async(req, res)=>{
    try {
...
// add a new project
router.post('/addProject', [superUser, [
    check('name'), check('customerId'), check('technology'), check('start'), check('end')
]], async(req, res)=>{
...
// update a project
router.put('/updateProject/:project_id', [superUser, [
    check('name'), check('customerId'), check('technology'), check('start'), check('end')
]], async(req, res)=>{
    const {name, customerId, technology, start, end}=req.body
    const errors=[]
...
// delete a project
router.delete('/deleteProject/:project_id', superUser, async(req, res)=>{
    try {
        // check url project id
        if(!mongoose.Types.ObjectId.isValid(req.params.project_id)) return res.status(400).json({message: 'Invalid Project Id', success: false})
