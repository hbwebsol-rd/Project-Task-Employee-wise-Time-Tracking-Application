...
import {
  ...
  const Updatetask = ({ open, setOpen, row, classes }) => {
    ...
    useEffect(()=> {
        setTaskDetails({taskName: row.taskName, projectId: row.projectName, employeeId: row.employeeName, priority: row.priority, status:row.status  })
    }, [row])
    ...
    const onSubmit = () => {
      dispatch(UpdateTaskAction(row._id, taskDetails));
      setOpen(false);
    };
    function handleClose() {
      reset();
